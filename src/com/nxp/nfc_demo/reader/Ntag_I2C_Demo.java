...
//		LedFragment.setVoltage(0);
		LedFragment.setTempSens("Sensor From Arduino1");
	...
	private class LedTask extends AsyncTask<Void, Byte[], Void> {
		...
		protected Void doInBackground(Void... params) {
			byte[] dataTx = new byte[reader.getSRAMSize()]; //Initialize an array
			byte[] dataRx = new byte[reader.getSRAMSize()];
			byte[] Led;
			Byte[][] result;
...
//					}
					...
					double ard = LedFragment.getArduino();
					DecimalFormat df = new DecimalFormat("0");
					byte[] ard1 = df.format(ard).getBytes();

					dataTx[reader.getSRAMSize() - 60] = ard1[0];
			...
			//		Thread.sleep(10);
			//		reader.waitforI2Cread(DELAY_TIME);
					Log.d("Debug0", String.valueOf(reader.checkPTwritePossible()));
					Log.d("Debug1", "OK1");
					Log.d("Debug2", String.valueOf(dataTx.length));

					/*for(int i = 0; i < dataTx.length; i++) {
						Log.d("dataTx", String.valueOf(dataTx[i]));
					}*/
					Log.d("Debug3", String.valueOf(dataTx.length));
			//		reader.writeSRAMBlock(dataTx, null);
					...
					for(int i = 0; i < dataRx.length; i++) {
						Log.d("dataRx", String.valueOf(dataRx[i]));
					}
					Log.d("Debug5", String.valueOf(dataRx.length));
					Log.d("Debug6", "OK300");
/*					if (exit) {
...
//						dataTx[reader.getSRAMSize() - 3] = '0';
				...
				cancel(true);
				e.printStackTrace();
			} catch (IOException e) { //cai neste catch
				displayTransferDir(noTransfer);
				cancel(true);
				e.printStackTrace();
			} catch (CommandNotSupportedException e) {
...
//				LedFragment.setButton(bytes[1][reader.getSRAMSize() - 2]);
				//Log.d("SAD", String.valueOf(reader.getSRAMSize()));
//				int temp = 0;
				double value_ard = 0;
...
//				temp |= ((bytes[1][reader.getSRAMSize() - 6] << 3) & 0x000007F8);
				...
				//Arduino
				int msb = bytes[1][reader.getSRAMSize() - 64];
				int lsb = bytes[1][reader.getSRAMSize() - 63];

				if(lsb < 0) lsb = 256+lsb;
				
				value_ard = (msb << 11) |(lsb << 3);
				//value_ard |= (bytes[1][reader.getSRAMSize() - 63] << 3);
				value_ard *= 0.0078125f;
...
//							+ calcTempCelsius(temp) + " ºC / "
//							+ calcTempFarenheit(temp) + " °F /"
//							+ " OK1"
//							+ "\nEnergy Harvesting Voltage: "
//							+ calcVoltage(voltage));
//					LedFragment.setTempSens("OK2_" + value_ard);
//				} else {
...
//				}
				...
				LedFragment.setTempSens("Temperature: " + calcArduinoTemp(value_ard)+ " °C");
				Log.d("Val", String.valueOf(value_ard));
...
//				String Message = "Phone sends too fast or Tag was lost, try Polling mode";
	...
	/**
	 * Calculates the DS18B20 Temperature in Celsius .
	 *
	 * @param temp
	 *            Temperature
	 * @return String of Temperature in Dez
	 */
	private String calcArduinoTemp(double temp) {
		double tempDouble = 0;
		String tempString = "";
		// If the 11 Bit is 1 it is negative
//		if ((temp & (1 << 11)) == (1 << 11)) {
